...
Para obtener la exposici\'on a neutrinos del detector es necesario integrar las eficiencias de identificaci\'on en convoluci\'on con la probabilidad de ocurencia de cada uno de los eventos considerados.
Para obtener las eficiencias se utilizaron los criterios de seleci\'on delineados en el cap\'itulo pasado, donde se los aplic\'o en conjunto a todas las simulaciones para obtener, luego de integrar, la exposici\'on combinada de los tres an\'alisis.
Por otro lado las probabilidades surgieron de un an\'alisis del mecanismo de detecci\'on y, en ES, del uso de simulaciones de Monte Carlo.
Para llevar a cabo la integraci\'on temporal y en \'area se utiliz\'o una t\'ecnica que permite tener en cuenta las fluctuaciones del arreglo a trav\'es del tiempo, donde en particular, se desarroll\'o un m\'etodo para incluir el cambio en las eficiencias que causa el envejecimiento del detector.
Finalmente se obtuvieron las incertezas sistem\'aticas cometidas en las componentes m\'as relevantes del c\'alculo y se presenta su combinaci\'on hasta el resultado final.
...
% 	
	...
	Mientras que el rápido crecimiento inicial se encuentra dominado por una única reflexión de la luz Cherenkov en el Tyvek de la estación, el decaimiento posterior se produce debido a varias de estas reflexiones \cite{icrc}.
...
Una vez aplicados se utiliza la exposici\'on y sus incertezas para informar el valor m\'as probable de $k$ con su intervalo de confianza.
	...
	\begin{itemize}
	 \item \textbf{ES}: el 1 de enero al 20 de junio de 2013;
	 \item \textbf{DGH}: del 1 de junio de 2010 al 20 de junio de 2013;
	 \item \textbf{DGL}: 1 de enero de 2012 al 20 de junio de 2013.
	\end{itemize}

	En las figuras \ref{fig:unblindingES} a \ref{fig:unblindingDGL} se muestran las distribuciones de variables discriminantes para los tres an\'alisis, para la muestra de b\'usqueda actual y para la \'ultima muestra escrutada previo a este trabajo, junto a los cortes de selecci\'on.
...
% 	\end{figure}
	...
	Considerando que se observaron 0 eventos, asumido de manera conservadora 0 eventos de fondo esperado y requiriendo un nivel de confianza de \cant{90\%}{C.L.}, se obtuvo $N_{up}=2.39$.
	Con este valor, y a partir de la ecuaci\'on \ref{eq:kup}, se calcul\'o una cota superior al flujo de neutrinos, que aplica en el rango energ\'etico de\linebreak \cant{1.0\times10^{17}}{eV} - \cant{2.5\times10^{19}}{eV}:
	%
	\begin{equation}
	...
	\end{equation}
	%
	Este rango es en el que se espera haber detectado el \cant{90}{\%} de los eventos.
	...
	Por otro lado, es posible integrar el denominador de la ecuaci\'on \ref{eq:kup} en distintos intervalos de energ\'ia, obteniendo as\'i un l\'imite superior $k_{up}$ en dicho intervalo, lo que se denomina un l\'imite diferencial.
	...
	%
 	...
 	Para obtener cada uno de estos n\'umeros se integr\'o la ecuaci\'on \ref{eq:expTot} utilizando el flujo correspondiente y la exposici\'on de la figura \ref{fig:expTot}.
 	%----------------------------------------------------------------------------------------
	\begin{table*}[!t]
	\begin{center}
