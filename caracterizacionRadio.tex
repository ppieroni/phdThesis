...
% 	 ue ser\'a transmitida r\'apidamente a las nuevas generaciones de part\'iculas. Por otro lado, la energ\'ia transferida a part\'iculas penetrantes, neutrinos y muones, se perder\'a debido a su baja frecuencia de interacci\'on.};
	...
	Dado que el impacto del efecto geomagn\'etico depende del producto $\vec{\beta}\times\vec{B}$, estas dos cualidades del campo magn\'etico en Tunka favorecen la detecci\'on de eventos inclinados.
	...
	La primer caracter\'istica importante de este tipo de eventos es la geometr\'ia de la huella dejada sobre el detector.
	...
	\begin{figure}[ht!]
		...
		Huella de campo eléctrico generada por la lluvia ES de la tabla \ref{tab:paramTestShower} en el sitio Tunka (tabla \ref{tab:possibleLoc}). La lluvia se propaga de izquierda a derecha en este gr\'afico. En rojo se muestra la zona de impacto del cono \cher{} si el máximo de la lluvia se encuentra a \cant{11}{km} del punto de decaimiento. Esta curva se obtuvo a partir de la ecuación \ref{eq:conewidth} (cap\'itulo \ref{ch:simulacionRadio}), a la que se llegó mediante hipótesis geométricas.
		}
	\end{figure}
	%
	...
	Si bien \zhs{} realiza la simulación microscópicamente, es decir, sin realizar ninguna suposición sobre los mecanismos macrosc\'opicos, este resultado permite corroborar que la geometr\'ia de la emisi\'on de radio en eventos ES se explica a primer orden mediante los razonamientos delineados en la sección \ref{sbsc:geom_emision}.
	...
	\begin{figure}[ht!]
		...
		Huella de la componente del campo eléctrico en la dirección ${\rm TR}$ (direcci\'on norte sur, ver figura \ref{fig:malField}). La intensidad del cono \cher{} es distinta a ambos lados del eje de la lluvia.
		Cuando la coordenada transversal es positiva (al norte) el efecto Askaryan y el geomagnético se suprimen mientras que cuando es negativa (al sur) se favorecen.
		}
	\end{figure}
	%
	Como puede observarse en la figura \ref{fig:testFootprint_E0tr}, la intensidad del cono \cher{} en direcci\'on ${\rm TR}$ no es simétrica respecto del eje de la lluvia.
	...
	\begin{figure}[ht!]
		...
		Huella de la componente del campo eléctrico en la dirección $z$ (vertical al suelo). En esta dirección las contribuciones geomagnética y Askaryan se suman en cualquier punto del suelo.
		}
	\end{figure}
	%
	Por otro lado en \ref{fig:testFootprint_E0z} la componente $z$ del campo muestra una huella mucho más simétrica respecto del eje generado por la direcci\'on de propagaci\'on, consistente con que ambas contribuciones (geomagn\'etica y Askaryan) predicen una componente $z>0$ en toda la superficie.
	...
	\begin{figure}[ht!]
		...
		Huella de la componente del campo eléctrico en la dirección ${\rm FW}$ (direcci\'on este-oeste, ver figura \ref{fig:malField}). Su intensidad es dos \'ordenes de magnitud menor que en las direcciones ${\rm TR}$ y $z$ de las figuras \ref{fig:testFootprint_E0tr} y \ref{fig:testFootprint_E0z}.
		}
	\end{figure}
	%
	...
	En la figura \ref{fig:footprintPolariz} se muestra la distribuci\'on de \'angulo azimutal ($0^\circ$ hacia el norte, ver figura \ref{fig:malField}) y cenital del campo para las antenas cuya se\~nal supera los \cant{50}{\mu V/m}.
	%
	\begin{figure}[ht!]
		\centering
		...
		Valor del \'angulo azimutal (panel superior) y cenital (panel inferior) del campo el\'ectrico a lo largo de la huella de la lluvia. Se observa que el \'angulo azimutal se mantiene en valores \emph{cercanos} a cero (el campo apunta hacia el norte) mientras que el cenital no presenta var\'ia entre $0^\circ$ y $90^\circ$.
		}
	\end{figure}
	%
	...
	Para definir esta restricci\'on resulta \'util la figura \ref{fig:phiDist}, que muestra la distribuci\'on de $\phi$ en forma de histograma. 
	%
	\begin{figure}[ht!]
		\centering
		\includegraphics[width=\textwidth]{./fig/simulacionRadio/{phiDist_Test_18_89.5_90_25_1238}.png}
		\caption{\label{fig:phiDist}
		Histograma del \'angulo azimutal del evento de la tabla \ref{tab:paramTestShower}. Se distinguen las entradas procedentes de la zona norte (sur) de la huella en las que se observan valores levemente sesgados negativamente (positivamente). Se observa que la mayor\'ia de las antenas presentan valores entre $-2.5^\circ$ y $2.5^\circ$, lo que da una idea del \'orden de magnitud del rango angular a seleccionar.
		}
	\end{figure}
	%
	...
	Como consecuencia esta ganancia permitir\'ia bajar el nivel de disparo local de las antenas, aumentando la eficiencia a baja energ\'ia (ver secci\'on \ref{sbsc:depEvRadio}).
...
% 	
	...
	En la secci\'on \ref{sbsc:localTriggerRadio} se decidi\'o que la condici\'on de disparo de cada antena dependa del campo el\'ectrico m\'aximo registrado por la misma.
...
% 	Este comportamiento tambien se observa para diferentes alturas de decaimiento del \tauon{}
	...
	Adem\'as, como se ver\'a mas adelante, este efecto tendr\'a importancia s\'olo a baja energ\'ia, mientras que para energ\'ias superiores a \cant{E_v\sim10^{18}}{eV} la eficiencia a altos \xd{} se ver\'a suprimida por el efecto de la curvatura de la tierra (ver ap\'endice \ref{ap:tierraCurva} y cap\'itulo \ref{ch:resultadosRadio}).
	...
	Con esto en mente resulta interesante entonces observar como se modifica la huella de $|E|$ del evento de referencia al cambiar el \'angulo azimutal entre $\phi=0^\circ$ y $\phi=90^\circ$, lo que se muestra en la figura \ref{fig:phi_dependence}. 
	\begin{figure}[ht!]
		\centering
		\begin{tabular}{cc}
		$0^\circ$ & $90^\circ$ \\
	...
	\subsection{Efecto de la energ\'ia visible $E_v$}
...
% 	Dado que el par\'ametro de control de energ\'ia elegido en esta parte de la tesis es la energ\'ia visible, es decir, la que portan las part\'iculas poco penetrantes al inicio de la lluvia, el canal de decaimiento queda determinado por las part\'iculas la inician y por como se distribuye la energ\'ia entre ellas.
% 	\footnote{Como contraparte, en el an\'alisis para Auger lo que determinaba el decaimiento del \tauon{}, ademas de las part\'iculas y la distribuci\'on de energ\'ia, es qu\'e fracci\'on de la energ\'ia del tau\'on se transmitia a las part\'iculas interactuantes.}.
	...
	\begin{figure}[ht!]
		...
		Distribuci\'on de variables de la lluvia relevantes en la detecci\'on, para 30 decaimientos del \tauon{} tomados al azar y para dos alturas de decaimiento diferentes, \cant{{\rm x_d}=25}{m} y \cant{{\rm x_d}=150}{m}. Arriba: campo el\'ectrico m\'aximo. Medio: \'area cubierta por un campo el\'ectrico que supera el $80\%$ del m\'aximo de la huella. Abajo: ancho promedio de la lluvia. En cada distribuci\'on se detalla la posici\'on del evento de referencia de la tabla \ref{tab:paramTestShower}. 
		}
	\end{figure}
	%
...
% 	El m\'aximo de la huella se obtuvo simplemente como la m\'axima se\~nal registrada por las antenas simuladas.
	...
	Por otro lado el c\'alculo del ancho promedio de la huella requiri\'o tres pasos.
