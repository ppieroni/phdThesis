...
Asimismo, la distancia entre antenas no debe ser inferior a \cant{1000}{m} y \cant{100}{m} respectivamente, lo que implica que cada evento considerado necesita de la simulaci\'on de alrededor de 2000 antenas.
...
El primer paso entonces fue estudiar la probabilidad relativa de ocurrencia de los eventos como funci\'on de $E_v$, $\theta_D$ y ${\rm x_d}$, lo que se desarrolla a continuaci\'on.
	...
	No obstante, este \'ultimo factor parece ser el m\'as relevante ya que entre \cant{10^{16.25}}{eV} y \cant{10^{18.25}}{eV} los pesos caen cerca de dos \'ordenes de magnitud.
	...
	\begin{figure}[ht!]
		...
		Estos gr\'aficos muestran la importancia relativa en la exposici\'on de los diferentes bines de energ\'ia visibles, \'angulos cenitales y alturas de decaimiento.
 		Por ejemplo, para $\theta_D\sim90^\circ$ y \cant{ {\rm x_d}=150}{m} se esperan del orden de 100 veces m\'as eventos a \cant{E_v=10^{17.25}}{eV} que a \cant{E_v=10^{18.25}}{eV}.
		}
	\end{figure}
...
% 	 \item Se corrigi\'o el tiempo del m\'aximo de la se\~nal, que luego ser\'a el tiempo de disparo, seg\'un la distancia en la direcci\'on forward, en unidades de la velocidad de la luz.
	 \item Para evaluar la condici\'on de disparo local se utiliz\'o un umbral fijo para el pico del campo el\'ectrico, de acuerdo con lo discutido en la secci\'on \ref{sbsc:localTriggerRadio}, que vari\'o entre \cant{25}{\frac{\mu V}{m}} y \cant{250}{\frac{\mu V}{m}}.
	...
	En las figuras \ref{fig:corePos1} y \ref{fig:corePos2} se muestran las posiciones generadas para una de las lluvias en distintas topograf\'ias.
	...
	\end{figure}
	%
	Cada punto negro representa una antena del detector, mientras que la zona azul corresponde a la regi\'on en la que la condici\'on de disparo local fue alcanzada, en este caso correspondiente a un campo umbral de \cant{100}{\frac{\mu V}{m}}.
	...
	En primer lugar para cada valor de \xd{} aparece el \'angulo cenital m\'inimo $\theta_D^{cut}$ por encima del cual la eficiencia es 0.
	...
	Otro detalle que resulta notable es que por una peque\~na variaci\'on del \'angulo cenital de aproximadamente $0.25^\circ$ los eventos pasan de saturar la eficiencia a no producirse ning\'un disparo del detector.
	...
	Hasta el momento lo expuesto es una conjetura basada en los resultados del cap\'itulo \ref{ch:caracterizacionRadio}.
	...
	Por otro lado su ancho var\'ia entre \cant{4000}{m} a \cant{8000}{m} lo que supera el observado en eventos ES, que ronda los \cant{2000}{m} (ver figura \ref{fig:testFootprint_Cone} del cap\'itulo \ref{ch:caracterizacionRadio}).
...
\section{Desempe\~no de un detector de 90000 antenas}
	...
	Para conciliar en primer lugar el tiempo de medici\'on se calcul\'o $N_{Radio}$ imponiendo \cant{T=1}{s} en la ecuaci\'on \ref{eq:exp2ESRadio} mientras que $N_{Auger}$ fue calculado utilizando la exposici\'on \emph{Full Auger} a neutrinos ES para \cant{6.5}{yr} y se la dividi\'o por \cant{2.05\times10^8}{s}~\footnote{Este n\'umero corresponde a 6.5 a\~nos en segundos.}, obteniendo de esa manera la exposici\'on promedio por segundo del observatorio.
	...
	Si en vez de un arreglo regular se considera uno tipo panal de abeja, con \cant{a_1=a_2=1500}{m} y $\alpha=60^\circ$, la cuenta es levemente diferente. 
	...
	Esto quiere decir que al menos en estas condiciones de disparo local y global los t\'erminos $a_1 \times a_2$ y $\sin \alpha$ en el c\'alculo de la superficie cubierta por el detector se imponen sobre la p\'erdida de eficiencia que podr\'ia provocar la separaci\'on de las antenas.
	De igual forma, en la topograf\'ia DE (figura \ref{fig:compRadioAuger_de}) el n\'umero de eventos esperados aumenta con $d$ y $D$, lo que indica nuevamente que el incremento en el \'area termina imponiendose por sobre la p\'erdida de eficiencia.
	...
	Esto no es siempre de esta manera, por ejemplo en la figura \ref{fig:compRadioAuger_th2} se muestra la cantidad de eventos como funci\'on de los pasos $a_1$ y $a_2$ para varios valores de $\alpha$ en un arreglo regular, para un threshold local de \cant{100}{\mu V/m} y uno global de 9 antenas.
	%
	\begin{figure}[h!]
		\begin{center}
			\textbf{\footnotesize Arreglo regular}\\
	...
	Para generalizar esta idea las figuras \ref{fig:compRadioAuger_th} y \ref{fig:compRadioAuger_th3} muestran c\'omo cambia la cantidad de eventos esperados y el valor medio de la energ\'ia (calculado con la ecuaci\'on \ref{eq:emean_radio}) en funci\'on del threshold local, para varios valores de $D$ en un arreglo tipo DE.
	%
	\begin{figure}[h!]
		\begin{center}
			\textbf{\footnotesize Arreglo de bordes densos}\\
			\includegraphics[width=\textwidth]{fig/resultadosRadio/perf/thresh/{CompRadioAuger_1000.0_5.0_1.0_de_modo3}.pdf}
			\caption{\label{fig:compRadioAuger_th} Numero de eventos esperados como funci\'on del nivel de disparo local y para distintos valores de $D$ en un arreglo de bordes densos. Se observa un decaimiento a medida que aumenta el threshold.}
		\end{center}
	\end{figure}
	%
	\begin{figure}[h!]
	...
	Entonces para encarar esta cuestion se calcul\'o para las tres topograf\'ias la superficie cubierta $S$ por cada detector simulado y la \emph{dimensi\'on equivalente} $L=\sqrt{S}$, que corresponde al lado de un detector cuadrado equivalente que cubre tal superficie.
	En la figura \ref{fig:compRadioAuger_area0} se muestra a modo de ejemplo $L$ como funci\'on de los par\'ametros $a_1=a_2$ y $\alpha$ para un arreglo regular.
	%
	\begin{figure}[h!]
		\begin{center}
	...
	\end{figure}
	%
	En l\'ineas negras se resalta la dimensi\'on equivalente de GRAND, y lo que aqu\'i se consider\'o como la m\'axima extensi\'on viable para la t\'ecnica, \cant{L=\sqrt{S}=500}{km}.
	...
	\small
		\begin{tabular}{|l|cc|cc|cc|}
			...
			\hline
		\end{tabular}
		\caption{\label{tab:bestConf} Configuraciones simuladas que arrojan mayor cantidad de eventos esperados bajo las condiciones \cant{L<500}{km} y \cant{L<250}{km} para cada una de las topograf\'ias. Se calcul\'o la exposici\'on para cada una de ellas.}
	\end{center}
	\end{table}
	
	\subsection{Perspectivas de la t\'ecnica}
	
	Para estudiar c\'omo se posiciona cada uno de los detectores considerados en la proxima generaci\'on de experimentos de neutrinos ultra energ\'eticos, se calcul\'o el limite diferencial al \cant{90\%}{C.L.} que podr\'ia imponer cada uno en caso de observar 0 eventos.
	Para ello se sigui\'o el procedimiento descripto en la secci\'on \ref{sc:limiteAuger} del cap\'itulo \ref{ch:resAuger}, donde se utiliz\'o un escenario de $\Phi(E_\nu)= k\cdot E_\nu^{-2}$, una relaci\'on entre sabores $\nu_e:\nu_\mu:\nu_\tau=1:1:1$ y $N_{up} = 2.4$ en la ecuaci\'on \ref{eq:kup}, que proviene de aplicar el m\'etodo de Feldman-Cousins con 0 eventos detectados y en ausencia de background.
...
% 			No se discrimina los resultados por topograf\'ia debido a que no se alcanzan a distinguir en la escala del gr\'afico.}
	...
	En la escala del dibujo las curvas correspondientes a las tres topograf\'ias son casi indistinguibles, por lo que no se las discrimin\'o.
	En l\'ineas verde, naranja y magenta se muestran los l\'imites publicados por IceCube, Auger y Anita-II respectivamente. 
	Las bandas azul y verde muestran las predicciones para ARA y ARIANNA en tres a\~nos de medici\'on.
	Se observa que mientras qu\'e un detector con \cant{L=250}{km} presenta un desempe\~no algo inferior al predicho para los detectores ARA~\cite{cite:ARA2} y ARIANNA\cite{cite:ARIANNA3}, el SD de \cant{L=500}{km} los supera a energ\'ias menores a \cant{10^{18}}{eV}.
	Esto demuestra que la t\'ecnica puede ser competitiva con la pr\'oxima generaci\'on de detectores de neutrinos c\'osmicos de ultra-energ\'eticos que se encuentran en construcci\'on.
	
	Por otro lado, las tablas \ref{tab:nRadio250} y \ref{tab:nRadio500} presentan la cantidad de eventos esperados en 3 a\~nos de funcionamiento, para cada topograf\'ia, para varios modelos de producci\'on de neutrinos cosmog\'enicos y para las extrapolaciones de los neutrinos observados por IceCube.
	...
	\small
		...
		IceCube extrapolado \emph{Best fit} \cite{cite:IceCubeFlux}  &      12.9 &             14.9 &            15   \\
			\hline
		\end{tabular}
		\caption{\label{tab:nRadio250} N\'umero de eventos esperados $N_{esp}$ en 3 a\~nos de funcionamiento seg\'un la ecuaci\'on \ref{eq:nesp_radio} para varios modelos de producci\'on de neutrinos y utilizando la exposici\'on elegida en cada tipo de topolog\'ia para un detector con \cant{L\sim250}{km}.}
	\end{center}
	\end{table}
	%
	...
	\small
		...
		IceCube extrapolado \emph{Best fit} \cite{cite:IceCubeFlux}  &    52.4 &  53.8  &   53.3 \\
			\hline
		\end{tabular}
		\caption{\label{tab:nRadio500} N\'umero de eventos esperados $N_{esp}$ en 3 a\~nos de funcionamiento seg\'un la ecuaci\'on \ref{eq:nesp_radio} para varios modelos de producci\'on de neutrinos y utilizando la exposici\'on elegida en cada tipo de topolog\'ia para un detector con \cant{L\sim500}{km}.}
	\end{center}
	\end{table}
	%
	...
	Por \'ultimo es posible estudiar los l\'imites de la t\'ecnica para detectores de los tama\~nos ya considerados.
	Para ello se utiliz\'o la exposici\'on de un detector ideal que observa un rango angular $\theta_D\in(90^\circ,92.5^\circ)$ (ver secci\'on \ref{sc:idealRadio}), cuyo tama\~no varia entre \cant{L=250}{km} y \cant{L=500}{km}, y que mide durante 3 a\~nos.
	...
	El resultado se muestra en la figura \ref{fig:limitesRadio_Full} junto a los l\'imites vigentes de IceCube, Auger y Anita-II y a las predicciones calculadas para ARA y ARIANNA.
	%
	\begin{figure}[h!]
		\begin{center}
	...
	\end{figure}
	%
	En esta figura el borde superior de la franja violeta corresponde a un detector de \cant{L=250}{km} y el l\'imite inferior a \cant{L=500}{km}.
	Si se comparan los bordes de la franja violeta con los de la rosa se observa a simple vista que el detector de radio tiene baja eficiencia para eventos de baja energ\'ia~\footnote{Esto puede inferirse debido a que hacia bajas energ\'ias la franja roja resulta mucho m\'as empinada que la violeta. Si a una dada energ\'ia el limite impuesto es relativamente alto, quiere decir que el detector no tiene buen desempe\~no detectando neutrinos de esa energ\'ia.}. 
	
	Por otro lado si tiene en cuenta en que detectores m\'as alla de los tama\~nos considerados se vuelven inviables, y que la eficiencia de disparo con los niveles de disparo local utilizados se suprime por encima de los $92.5^\circ$, esta banda representa los m\'aximos l\'imites diferenciales a neutrinos que pueden ser impuestos mediante arreglos de superficie de antenas de radio a nivel del suelo y para eventos ES.
	...
	En la tabla \ref{tab:nRadio_Full} se muestra esta cantidad para detectores de \cant{L=250}{km} y \cant{L=500}{km}.
	%
	\begin{table}[ht!] %%% fig/graficadores/nEvtRadio$ ./plotNEvtRadio_Max.py
	\begin{center}
	\renewcommand{\arraystretch}{1.3}
	\small
		...
		IceCube extrapolado \emph{Best fit} \cite{cite:IceCubeFlux}  &     280.2 &     70   \\
			\hline
		\end{tabular}
		\caption{\label{tab:nRadio_Full} N\'umero de eventos esperados $N_{esp}$ en 3 a\~nos de medici\'on seg\'un la ecuaci\'on \ref{eq:nesp_radio} para varios modelos de producci\'on de neutrinos y utilizando la exposici\'on de un detecter $100\%$ eficiente cuya dimensi\'on caracter\'istica es \cant{L\sim250}{km} y \cant{L=500}{km}.}
	\end{center}
	\end{table}
	...
	En los tres casos las eficiencias pueden incrementarse bajando el nivel de disparo local de las antenas (por ejemplo ver figura \ref{fig:compRadioAuger_th}). 
