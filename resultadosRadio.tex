...
% 	\end{figure}
	...
	Para cada se obtuvo la eficiencia de disparo en funci\'on de $E_v$, $\theta_D$ y ${\rm x_d}$.
	Para descontar el canal de decaimiento $\tau^{-}\rightarrow \mu^{-}\bar{\nu_{\mu}}\nu_{\tau}$ se multiplic\'o cada valor por un factor $0.826$.
	De esta manera se obtuvieron curvas como las de la figura \ref{fig:effRadio} en la que se grafican las eficiencias de disparo para un detector regular de par\'ametros \cant{a_1=a_2=1500}{m} y $\alpha=60^\circ$ y para un nivel de disparo local de \cant{75}{\frac{\mu V}{m}}.
	%
	\begin{figure}[h!]
		\begin{center}
	...
	\end{figure}
	%
	Como se esperaba seg\'un lo discutido en el cap\'itulo \ref{ch:caracterizacionRadio} la eficiencia de disparo global aumenta con la energ\'ia visible.
	...
	Una vez obtenidas las curvas fue posible comparar cada detector con el desempe\~no alcanzado por Auger.
	Para ello, se calcul\'o la integral \ref{eq:exp2ESRadio} utilizando las eficiencias de Auger y se agreg\'o un factor de correcci\'on por la diferencia de superficie cubierta por la celda primitiva y la cantidad de celdas en el detector.
	La informaci\'on necesaria para realizar dicha conversi\'on se resume en la tabla  \ref{tab:conv2Auger}.
	\begin{table}
	\centering
	...
	\begin{tabular}{cccc}
	 ...
	 Panal de abeja &&$\frac{2}{3}$&90000\\
	 Bordes densos &$D^2$&$\frac{2D}{d}-1$& $\dfrac{90000}{\frac{2D}{d}-1}$
	\end{tabular}
	\caption{\label{tab:conv2Auger} Factores necesarios para comparar detectores con diferentes topograf\'ias.
	}
	\end{table}
	
	Con todo esto se realizaron gr\'aficos como los de la figura \ref{fig:compRadioAuger}, en la que se muestra la relaci\'on entre los eventos esperados para un detector cuyos par\'ametros 
