...
\section{Desempe\~no de un detector de 90000 antenas}
	...
	En esta l\'inea una vez definidos los criterios y m\'etodos que permiten calcular las eficiencias de detecci\'on ser\'a posible en primer lugar estudiar qu\'e aspectos del dise\~no del detector son relevantes y en qu\'e medida impactan sobre la medici\'on.
	Al mismo tiempo se utilizar\'a toda la informaci\'on recolectada, junto con la ecuaci\'on \ref{eq:exp2ESRadio}, para obtener l\'imites diferenciales al 90$\%$ de nivel de confianza que podr\'ia imponer el detector y se lo comparar\'a con la proxima generaci\'on de detectores de neutrinos.
	...
	Sin embargo, existen caracter\'isticas a ls que les quedan grados de libertad, a saber:
	%
	\begin{enumerate}
	...
	Adem\'as, con el fin de enmarcar el resultado se informar\'a esta cantidad en unidades de $N_{Auger}$ (o sea $N_{Radio}/N_{Auger}$), que representa los eventos ES que detectar\'ia el SD Auger en una medici\'on equivalente\footnote{Mismo flujo y misma duraci\'on.}.
	...
	La topograf\'ia del detector de tipo regular (RE) y panal de abeja (HC) queda definida por los par\'ametros $\left\{a_1,a_2,\alpha\right\}$ mientras que en la de bordes densos (DE) por el conjunto $\left\{d,D\right\}$.
	Sin embargo, debido a que en topograf\'ias RE y HC las configuraciones con $a_1$ muy distinto de $a_2$ (dentro de valores razonables) pueden lograrse aproximadamente imponiendo $a_1=a_2$ y un valor adecuado para $\alpha$, para acotar el n\'umero de configuraciones consideradas s\'olo se estudiaron casos en los que se cumple la condici\'on $a_1=a_2$.
	Por todo esto en todos los casos (RE, HC y DE) bast\'o dar 2 n\'umeros para determinar completamente la disposici\'on de antenas sobre la superficie del detector.
% 	
% 	
...
% 	Para ello, se calcul\'o la integral de la ecuaci\'on \ref{eq:exp2ESRadio} utilizando las eficiencias de Auger y se agreg\'o un factor de correcci\'on para incorporar la diferencia de superficie cubierta por cada celda primitiva y la diferencia en la cantidad de celdas en el detector.
	...
	Adem\'as de los par\'ametros que determinan la configuraci\'on, para poder comparar las cantidades $N_{Radio}$ y $N_{Auger}$ de manera justa fue necesario calcular sus valores en tiempos de medici\'on equivalentes y aplicar un factor de correcci\'on que tenga en cuenta la diferencia de tama\~no de los detectores.
	Para conciliar en primer lugar el tiempo de medici\'on se calcul\'o $N_{Radio}$ imponiendo \cant{T=1}{s} en la ecuaci\'on \ref{eq:exp2ESRadio} mientras que $N_{Auger}$ fue calculado utilzando la exposici\'on \emph{Full Auger} a neutrinos ES para \cant{6.5}{yr} y se la dividi\'o por \cant{2.05\times10^8}{s}, obteniendo de esa manera la exposici\'on promedio por segundo del observatorio.
	Luego, para incluir la diferencia de tama\~no de los detectores en cada configuraci\'on se determin\'o la celda primitiva correspondiente, se calcu\'o su \'area y el n\'umero de celdas que lo conforman para al fin calcular la superficie cubierta por el detector.
	Para el SD de Auger por ejemplo la superficie de la celda primitiva se puede calcular como \cant{1500^2\sin60^\circ}{m^2}=\cant{1.94\times10^6}{m^2}, donde cada una contiene una sola estaci\'on.
	...
	Por ende, en este caso el n\'umero de eventos extra debido a la diferencia de superficie instrumentada es alrededor de $56.25$ m\'as a favor del detector de radio.
	Por otro lado, si en vez de un areglo regular se considera uno tipo panal de abeja, con \cant{a_1=a_2=1500}{m} y $\alpha=60^\circ$, la cuenta es levemente diferente. 
	En este caso en lugar de utilizar la celda de Wigner-Seitz, como en la figura \ref{fig:corePos2}, es posible explotar el hecho de que un arrego HC es uno RE en el que se quita un nodo cada tres.
	...
	En estas condiciones las figuras \ref{fig:compRadioAuger_re}, \ref{fig:compRadioAuger_hc} y \ref{fig:compRadioAuger_de}, muestan $N_{Radio}/N_{Auger}$ para diferentes combinaciones de par\'ametros para las topograf\'ias RE, HC y DE respectivamente. 
	Como nivel de disparo local se utiliz\'o \cant{50}{\mu V/m}, como disparo global 5 antenas disparadas (AoT) y un nivel de identificaci\'on hipot\'etico de 0.9.
	%
	\begin{figure}[h!]
		\begin{center}
	...
	Para analizar el impacto de cada uno de los par\'ametros en el cociente de eventos es fundamental comprender c\'omo afecta un cambio en la topograf\'ia al c\'alculo de la exposici\'on. En esta l\'inea es posible aislar dos efectos que compiten entre s\'i:
	%
	\begin{enumerate}
	...
	\end{enumerate}
	%
	Sin embargo el detalle de la topograf\'ia podr\'ia en principio compensar o profundizar cualquiera de los dos efectos.
	
	- analizar cada uno de los casos, contar como cambia el ratio N/N para cada tipo de topografia en funcion de los parametros.
	
	- notar que siempre que aumenta el area aumenta la cantidad de eventos.
	
	- mostrar que si el threshold es mas alto puede ser que esto no pase, osea, la perdida de granularidad puede pasar a ser impotante. POr ahi esto puede ir mas adelante, en la seccion de threshold.
	...
	-justificar que a thresholds bajos lo que importa es el area, notar que el \'area cubierta termina siendo el factor determinante lo que lleva a la siguiente subseccion
	
	\subsubsection{Limitaci\'on en extensi\'on}
	
	- dado que el factor importante es el area hay que fijarla en un valor razonable.
	- entonces miramos el L=sqrt(area) donde sabemos que grand es 250km y como maximo puede ser 500km.
	%
	\begin{figure}[h!]
		\begin{center}
			\includegraphics[width=\textwidth]{fig/resultadosRadio/perf/area/{Area_re}.pdf}
			\caption{\label{fig:compRadioAuger_area1} Lado equivalente de un SD cuadrado de topograf\'ia regular de par\'ametros $a_1=a_2$ y $\alpha$. En l\'ineas negras se marca el valor de $L$ correspondiente a GRAND, \cant{250}{km}, y el considerado como m\'aximo razonable en esta tesis, \cant{500}{km}.}
		\end{center}
	\end{figure}
	%
	
	- Entonces, usamos la informacion de esta figura para determinar cuales de las confguraciones simuladas son viables, y se muestran en la figura que sigue
	...
	- Ademas de la topografia del detectro otro aspecto clave de la detecci\'on es el nivel de disparo local
	- en la figura tal se muestra como cambia el ratio N/N para un arreglo regular y para diferentes valores de a1 a2 a tita fijo
	- se observa como a medida que aumenta el threshold la cantidad de eventos esperados baja, lo que se debe funamentalmente a la perdida de eventos de baja energia, comparar con la figura del capitulo de caracterizacion que muestra como es el maximo de campo electrico en funcion de la energia.
	...
	- Para sustentar que la perdida de eventos de baja energia es lo que impacta al variar el threshold se muestra la siguiente figura que marca como a medida que aumenta el threshold local la energia media de la ecuacion tal sube.
	- dado que las energia altas son full efficient, la perdida no puede venir de otro lado que no sea la perdida de las bajas energias.
	...
	- Mostrar como a medida que la cantidad de antenas sube, la energia media sube y como la cantidad de eventos esperados baja, y justificar de nuevo que la perdida es por los de baja energia qu etienen footprints mas chicos. referir a alguna figura del capitulod e caracteriszacion.
	...
	- separar los limites de full efficient y no full efficient? 
	- posibles 4 tablas, para 250 y para 500 km, para este calculo y para full efficient las tablas de eventos esperados para lso distintos modelos.
	- agregar todo esto en las conclusiones! mirar el mail de jaime para justificar que grand esta empezando a tocar hierro.
	- posible grafico de limites integrados.
