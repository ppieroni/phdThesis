...
#    #integral limit
#    intLimit = 2.4/np.trapz(np.multiply(data[13][:,1]*time*area,10**(2*(9-data[13][:,0]))),x=10**(9-data[13][:,0]))
#    print intLimit
#    aux = plt.plot([10**16.5 , 10**19.3],[-intLimit,-intLimit],color="r",ls='-',lw=3)
#    plots.append(aux)
#    lgnds.append('Integral limit 500km')
...
#    aux, = plt.plot(list(10**data[14][:,0]) + list(10**data[14][:,0])[::-1] + [list(10**data[14][:,0])[0]],diffRadio,color='darkviolet',ls='--',lw=2)
#    aux = patches.Rectangle((0,0),1,1,color='darkviolet',alpha=1.,lw=2,fill=False)
#    plots.append(aux)
#    lgnds.append(names[14])
    ...
    plt.savefig("limits_future_v1.2.pdf",format="pdf")
