...
def main(argv):
    ...
    area = 200000*200000*np.sin(90*kdeg2rad) * 1.5 * 1e4 # area de la celda primitiva * hclattice (1.5) * 1e4 celdas
    factor = 10**(9-data[12][:,0]) #E in GeV
    NRadio = np.multiply(data[12][:,1]*time*area,factor)
    diffRadio = map(lambda x: 1/x,NRadio)
    #~ aux, = plt.plot(10**data[12][:,0],diffRadio,color="red",ls='-',lw=0)
    plt.plot(10**data[12][:,0],diffRadio,color="r",ls='--',lw=0)
    plt.fill_between(10**data[12][:,0],diffRadio,color="r",alpha=0.4)
    aux = plt.Rectangle((0,0),1,1,color="r",alpha=0.4)
    plots.append(aux)
    ...
    plt.savefig("limits_future.pdf",format="pdf")
