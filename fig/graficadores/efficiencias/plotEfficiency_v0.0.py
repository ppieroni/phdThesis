...
def main(argv):
    ...
    for fName in argv:
        ...
        for i in np.arange(data.shape[0]):
            res.append([float(th),float(an),float(a1),float(a2),float(al),float(ef),data[i,1],data[i,2],data[i,4],data[i,5],tp])
                ...
                for st in step:
                    for al in alph:
                        for ef in effs:
                            for en in ener:
                                for tp in topo:
                                    print th,an,st,al,ef,ef,en,tp
                                    #~ title = r'Threshold: '+str(th)+r'$\rm \mu Vm$ - Trigger: '+str(an)+r'aOT - ID eff: '+str(ef)
                                    #~ title = "$\log(\frac{"+str(en)+"}{EeV})$"
                                    title = "10^"+str(en)+"eV"
                                    print title
                                    name1 = "eff"+str(th)+"_"+str(an)+"_"+str(st)+"_"+str(st)+"_"+str(al)+"_"+str(ef)+"_"+str(en)+"_m1.pdf"
                                    ...
                                    legs = []
                                    for tt in thts:
                                        ncond = np.multiply(cond,res[:,7] == tt)
                                        asort = np.argsort(res[ncond,8])
                                        plot, = plt.plot(res[ncond,8][asort],res[ncond,9][asort],lw=2,color=cm.jet((tt-thts[0])/(thts[-1]-thts[0]),1))
                                        plts.append(plot)
                                        legs.append(str(tt))
                                    plt.legend(plts,legs,title=r'$\theta [deg]$')
                                    plt.xlabel(r'$\rm x_d$',fontsize=18)
                                    ...
                                    for xd in xdss:
                                        ...
                                        plot, = plt.plot(res[ncond,7][asort],res[ncond,9][asort],lw=2,color=cm.jet((xd-xdss[0])/(xdss[-1]-xdss[0]),1))
                                        plts.append(plot)
                                        legs.append(str(xd))
                                    plt.legend(plts,legs,title=r'$\rm xd [m]$',loc=2)
